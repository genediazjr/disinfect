...
exports.register = (server, options, next) => {
    ...
    const disinfect = (inputObj, options, firstPass, secondPass) => {
    ...
    server.ext('onPostAuth', (request, reply) => {
        ...
        if (request.payload || Object.keys(request.params).length || Object.keys(request.query).length) {
            ...
            if (Object.keys(request.query).length) {
                request.query = disinfect(request.query, request.route.settings.plugins._disinfect, 'disinfectQuery', 'querySanitizer');
            }

            if (Object.keys(request.params).length) {
                request.params = disinfect(request.params, request.route.settings.plugins._disinfect, 'disinfectParams', 'paramsSanitizer');
            }

            if (request.payload && Object.keys(request.payload).length) {
                request.payload = disinfect(request.payload, request.route.settings.plugins._disinfect, 'disinfectPayload', 'payloadSanitizer');
            }
        }
